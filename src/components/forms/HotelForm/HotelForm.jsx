            ...
            <div className={s.hotel__data}>
                {searchResults && searchResults.id && <div className={s.hotel__main}>
                    <div className={s.title__edit}>
                        ...
                        <Button type="link" onClick={()=>{setIsEditing(true)}} icon={<EditOutlined/>}>
                            Редактировать
                        </Button>
                    </div>
                    ...
                    <h2>Местоположение</h2>
                        ...
                        <p className={s.location__lon_lat}><b>Долгота и широта:</b> {searchResults.location.lon_lat[0]}, {searchResults.location.lon_lat[1]} </p>
                    </div>
                    <div className={s.hotel__photos}>
                        <h2>Фотографии</h2>
                        <div className={s.photos__cards}>
                            {searchResults.photos_url.map((url,idx)=>{
                                return <Card
                                key={idx}
                                style={{
                                    width: 220,
                                ...
                                cover={< img alt = "hotel photo" src = {url} />}>
                    ...
                    <Drawer title="Редактировать информацию об отеле" placement="right" size='large' onClose={()=>{setIsEditing(false); setEditData(searchResults)}} open={isEditing}>
                        ...
                        <div className={s.edit__form}>
                            ...
                            <div className={s.edit__item}>
                                ...
                                <Input value={editData.location.city} onChange={e=>{setEditData({...editData, location:{...editData.location, city: e.target.value}})}}/>
                                <Input value={editData.location.full} onChange={e=>{setEditData({...editData, location:{...editData.location, full: e.target.value}})}}/>
                            </div>
                            <div className={s.edit__item}>
                                <h3>Фотографии</h3>
                                <List
                                    ...
                                    renderItem={(item,itemidx) => <List.Item onClick={()=>setEditData({...editData, photos_url: editData.photos_url.filter((el,idx)=>idx != itemidx)})}>{item}</List.Item>}/>
                                <Space.Compact>
                                    <Input value={newPhotoURL} onChange={(e)=>{setNewPhotoURL(e.target.value)}}/>
                                    <Button type='primary' onClick={()=>{setEditData({...editData, photos_url: [...editData.photos_url, newPhotoURL]}); setNewPhotoURL('')}}>Добавить URL</Button>
                                </Space.Compact>
                            </div>
                            <div className={s.edit__item}>
                                <h3>Услуги</h3>
                                <List
                                    ...
                                    renderItem={(item, itemidx) => <List.Item onClick={()=>{setEditData({...editData, features: editData.features.filter((el,idx)=>idx != itemidx)})}}>{item}</List.Item>}/>
                                <Space.Compact>
                                    <Input value={newFeature} onChange={(e)=>{setNewFeature(e.target.value)}}/>
                                    <Button type='primary' onClick={()=>{setEditData({...editData, features: [...editData.features, newFeature]}); setNewFeature('')}}>Добавить услугу</Button>
                                </Space.Compact>
                            </div>
